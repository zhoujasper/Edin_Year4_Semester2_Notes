...
\begin{problemset}[Exercises]
  \item Let $ \{ a_n \}$ be a sequence of complex number such that the infinite product $\prod_{n=1}^\infty a_n$ converges to a nonzero limit $P$. Show that 
  \begin{enumerate}[(a)]
    \item The infinite product $\prod_{n=1}^{\infty}a^{-1}_n$ converges and
    \[
  ...
  \begin{solution}
    ...
    \end{enumerate}
  \end{solution}
  \item The Möbius function $\mu:\mathbb{N} \to \mathbb{C}$ is defined as follows:
  \[
    \mu(n) = \begin{cases}
      1 & \text{ if } n=1, \\
      (-1)^k & \text{ if } n=p_1\cdots p_k \text{ is a product of } k \text{ distinct primes}, \\
  ...
  \end{solution}
  \item Let $f(n)=\mu(n)$ denote the Möbius function as defined in the previous exercise and let $g(n)=1$ for all $n \in \mathbb{N}$. Show that
  \begin{enumerate}[(a)]
    \item \[
            f * g := \sum_{d|n} f(d) g\left(\frac{n}{d}\right) = \sum_{d|n} \mu(d) = \begin{cases}
              1 & \text{ if } n=1, \\
              0 & \text{ if } n>1.
            \end{cases}
          \]
  ...
  \begin{enumerate}[(a)]
    \item For $n \in \mathbb{N}$,
            \[
              \sum_{d|n} \phi(d) = n.
            \]
    \item Let $f(n) = \phi(n)$ and $g(n) = 1$ for all $n \in \mathbb{N}$. Show that
            \[
  ...
  \begin{solution}
    ...
    \end{enumerate}
  \end{solution}
  \item Let $f:\mathbb{N} \to \mathbb{C}$, then
  \[
    F(z) := \sum_{n=1}^{\infty} \frac{f(n)}{n^z}
  \]
  is called the \textit{Dirichlet series} associated to $f$. You may find that the previous exercises~\ref{ex:d_n_divisors} and~\ref{ex:euler_totient} give some examples of Dirichlet series and satisfies certain properties, which can be summarized in the following

  \textbf{Theorem}: Suppose $F(z) = \sum_{n=1}^{\infty} \frac{f(n)}{n^z}$ and $G(z) = \sum_{n=1}^{\infty} \frac{g(n)}{n^z}$ are two Dirichlet series that converge for some $z \in \mathbb{C}$. Then the product $F(z)G(z)$
  \[
  ...
  Now, with the following steps, prove the above theorem.
  \begin{enumerate}[(a)]
